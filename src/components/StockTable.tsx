...
export function StockTable({ type }: StockTableProps) {
  ...
  return (
    <Card className="border-none shadow-sm bg-white/50 backdrop-blur-sm overflow-hidden">
      <CardHeader className="bg-muted/30 border-b border-border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 py-6">
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={type === 'purchase' ? 5 : 6} className="h-32 text-center">
                ...
                <TableRow>
                  <TableCell colSpan={type === 'purchase' ? 5 : 6} className="h-32 text-center text-slate-400 font-medium">
                    {t('no_records')}
                  </TableCell>
                </TableRow>
              ) : (
